...
void inittimer()
{
    TimerPtr = TS_ScheduleTask( timerhandler,TICRATE, 1, NULL, TASK_CHANNEL_TIMER );
    TS_Dispatch();
}
            ...
            case 0:  //[0] (receive master sync buffer)
                ...
                if ((movefifoend[other]&(TIMERUPDATESIZ-1)) == 0)
                    for(i=connectpoint2[connecthead];i>=0;i=connectpoint2[i])
                    {
                        ...
                        if (i == myconnectindex)
                            otherminlag = (int32_t)((signed char)packbuf[j]);
                        j++;
                    }
...
{
    int32_t cposx,cposy,cposz,dst,j,fz,cz,hz,lz;
    short sect, cang, k, choriz,tsect;
    struct player_struct *p;
    int32_t tposx,tposy,tposz,dx,dy,thoriz,i;
    short tang;
