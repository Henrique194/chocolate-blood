...
{
    int i;
    for (i = kMaxChannels-1; Channel[i].at5 != 0; i--)
        if (i < 0)
...
void sndStartSample(char *pzSound, int nVolume, int nChannel)
{
    ...
    pChannel->at0 = FX_PlayRaw(pData, nSize, GetRate(1), 0, nVolume, nVolume, nVolume, nVolume, (uintptr_t)pChannel);
}

void sndStartSample(uint32_t nSound, int nVolume, int nChannel, QBOOL bLoop)
{
    if (FXDevice == -1)
        return;
    ...
    if (bLoop)
    {
        ...
        pChannel->at0 = FX_PlayLoopedRaw(pData, nSize, pData+nLoopStart, pData+nLoopEnd, rate,
            0, nVolume, nVolume, nVolume, nVolume, (uintptr_t)pChannel);
        pChannel->at4 |= 1;
    }
    else
    {
        pChannel->at0 = FX_PlayRaw(pData, nSize, GetRate(pEffect->format), 0, nVolume, nVolume, nVolume, nVolume, (uintptr_t)pChannel);
        pChannel->at4 &= ~1;
    }
}

void sndStartWavID(uint32_t nSound, int nVolume, int nChannel)
{
    if (FXDevice == -1)
        return;
    ...
    if (!pChannel->at5)
        return;
    char *pData = (char*)gSoundRes.Lock(pChannel->at5);
    pChannel->at0 = FX_PlayWAV(pData, 0, nVolume, nVolume, nVolume, nVolume, (uintptr_t)pChannel);
}
...
void sndStartWavDisk(char *pzFile, int nVolume, int nChannel)
{
    ...
    HANDLE h;
    if ((h = FindFirstFileA(pzFile, &find)) == INVALID_HANDLE_VALUE)
        return;
    char *pData = (char*)gSoundRes.Alloc(find.nFileSizeLow);
    if (!pData)
    {
        FindClose(h);
        return;
    }
    if (!FileLoad(pzFile, pData, find.nFileSizeLow))
    {
        FindClose(h);
        return;
    }
...
void sndInit(QBOOL netgame)
{
    bNetGame = netgame;
    const char *res = szSoundRes;
    if (!res)
        res = "SOUNDS.RFF";
