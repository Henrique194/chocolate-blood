...
void func_10148(void)
{
    ...
    const char *val = iniFile->GetKeyString("options", "Online", "");
    strncpy(temp, val, 20);
...
void func_10324(void)
{
    ...
    if (char_148EEC)
    {
        ...
        const char *um = iniFile->GetKeyString("options", "UserMapName", "");
...
void ParseOptions(void)
{
    ...
    if (bAddUserMap)
    {
        char buf1[_MAX_PATH+10];
        const char *pDir, *pFName, *pExt;
        MySplitPath(gUserMapFilename, buf1, &pDir, &pFName, &pExt);
        strcpy(UserPath, pDir);
...
void main(int argc, char** argv)
{
    ...
    uint32_t memoryAvailable = func_A8B50();
